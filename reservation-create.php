...
<section class="section"><div class="shell narrow"><div class="form-card">
    ...
    <form method="post" action="reservation-create.php?venue_id=<?= (int) $venueId ?>" novalidate>
        ...
        <div class="form-group"><label for="event-date">Data evenimentului</label><input id="event-date" name="event_date" type="date" min="<?= date('Y-m-d') ?>" value="<?= e($input['event_date']) ?>" data-venue-id="<?= (int) $venueId ?>" data-forecast-endpoint="weather-forecast.php" data-unavailable-dates='<?= e(json_encode(array_values($unavailableDates), JSON_UNESCAPED_SLASHES)) ?>' required><small id="date-availability" class="date-feedback" aria-live="polite">Selectează o dată pentru verificarea disponibilității.</small></div>
